...
object ContentType {
  ...
  case object Wildcard extends ContentType("*", "*", None)
  case class Custom(`type`: String, subType: String) extends ContentType(`type`, subType)
  ...
  def apply(contentType: String): ContentType = {
    contentType.toLowerCase.replaceAll(";(.*)", "").trim match {
      ...
      case "*/*" => Wildcard
      case t if t.contains("/") => Custom(t.substring(0, t.indexOf('/')), t.substring(t.indexOf('/') + 1))
